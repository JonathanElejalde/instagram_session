...
if __name__ == '__main__':
    ...
    if username in my_instagram.select(cursor, "User"):
        ...
        users_left = get_users_left(my_instagram, config.USERS_LEFT_PATH)
        unfollow_left = get_users_left(my_instagram, config.UNFOLLOW_LEFT_PATH)
    else:
    ...
    try:
        for i in range(len(users_left)):
            ...
            keyboardinterrupt(my_instagram, users_left, config.USERS_LEFT_PATH)
            
    except Exception as e:
        other_exception(e, my_instagram, users_left, config.USERS_LEFT_PATH)
